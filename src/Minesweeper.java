...
public class Minesweeper extends JFrame implements MouseListener{
	...
	static int mine_num;//variable for number of mine
	static int sign_num;//variable for number of flag
	static double score1;//variable for the basic score for Player1
	...
	static int bet1;
	static int bet2;
	static double score2;//Variable for the basic score for Player2
	static int [][]mine;//Save mine's location
	static int [][]mine1;//show the data
	int [][] sign;//save flag location
	int [][]uncertainty;// save the question mark location
	static BufferedImage image;//image path
	static int judge;//Determine the game state, in the game:0, win:1; lose:2
	static boolean mine_place;//Draw the switch of the mine to determine whether the mine has been placed, to prevent multiple placements
...
//	----------------------------------------------------------------
	...
	public Minesweeper() {
		...
		//window_width=560,window_height=640;//Initialize the game window size
		//mine_crosswise=10,mine_vertical=10;//Initialize minefield to 10*10
		mine_num=10;//Initialize number of mine
		sign_num=0;//Initialize number of flag
		score1=200;//Initialize the basic score for Player1
		numofclick=0;
		area1=0;
		area2=0;
		
		
		bet1=0;//save bet for player1
		bet2=0;//save bet for player2
		score2=200;//Initialize the basic score for Player2
		mine = new int [mine_crosswise][mine_vertical];//Save mine's location
		mine1=new int [mine_crosswise][mine_vertical];//show the data
		sign=new int [mine_crosswise][mine_vertical];//save flag location
		uncertainty=new int [mine_crosswise][mine_vertical];// save the question mark location
		image=null;//image path
		judge=0;//Determine the game state, in the game:0, win:1; lose:2
		mine_place=false;//Draw the switch of the mine to determine whether the mine has been placed, to prevent multiple placements
		...
		setBet(); //ask user for bet numbers
		this.addMouseListener(this);
        	
		
	}
	
	public static void setBet() {             //Method setBet():ask user for bet number
	...
	public void paint(Graphics g) {		
		...
		g2.setColor(Color.red);
		g2.setFont(new Font("Meiryo",8,20));
		g2.drawString("PLAYER1: "+score1,window_width -370, 80);
		g2.setColor(Color.red);
		g2.setFont(new Font("Meiryo",8,20));		
		...
		g2.setColor(Color.red);
		g2.setFont(new Font("Meiryo",8,20));
		g2.drawString("PLAYER2: "+score2,window_width - 200, 80);
		g2.setColor(Color.red);
		g2.setFont(new Font("Meiryo",8,20));
 ...
 */
	...
	public void mouseClicked(MouseEvent e) {
					...
					if(sign[x][y]==0&&uncertainty[x][y]==0) {
						...
						if(mine[x][y]!=9) {
							...
							if(numofclick%2==1) {
								score1-=bet1;
								score2+=bet1;
							JOptionPane.showMessageDialog(this,"Player1: BOOM  Player2: WIN!!");}
							else {score2-=bet2;
								score1+=bet2;
							JOptionPane.showMessageDialog(this,"Player2: BOOM  Player1: WIN!!");}
						}
		...
		else if(judge==1||judge==2) {    //if game finished, ask user if they want to replay. if yes, reset the game
			int answer=JOptionPane.showConfirmDialog(null, "Do you want to play again?");
			if(answer==0)//if answer=0, will replay the game
			{
			 
				this.dispose();//close current window			
				Welcome index=new Welcome();	//restart the game			
			}
			else
				{
				ImageIcon smile=new ImageIcon("C:\\Users\\sharon\\Desktop\\csc330\\CSC-330-GroupProject-Minesweeper-main\\img\\win.png");
				JOptionPane.showMessageDialog(null,"Thank you for playing Minesweeper!","Minesweeper",JOptionPane.PLAIN_MESSAGE,smile);
				this.dispose();//close current window
				}
